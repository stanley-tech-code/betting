...
export default function ZoneCenter() {
  ...
  useEffect(() => {
    async function fetchZones() {
      try {
        const res = await fetch('/api/zones/stats');
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-zinc-800">
            {loading ? (
              <tr><td colSpan={10} className="px-6 py-8 text-center text-zinc-500">Loading zones...</td></tr>
            ) : zones.length === 0 ? (
              <tr><td colSpan={10} className="px-6 py-8 text-center text-zinc-500">No zone data available. Sync PropellerAds data first.</td></tr>
            ) : (
              zones.map(z => (
                <tr key={z.zone_id} className="hover:bg-zinc-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <TierBadge tier={z.tier} />
                  </td>
                  <td className="px-6 py-4 font-mono text-zinc-300">#{z.zone_id}</td>
                  <td className="px-6 py-4 text-zinc-300">{z.campaign_id || 'N/A'}</td>
